app.controller('propertyController', [
	...
	function ($rootScope, $scope, $timeout, tokkoApi, $stateParams, getFeaturedProperties, utils, sliderMoves, $filter, $q) {
		...
		tokkoApi.find('property', id, $q.defer()).then(
			(result) => {
				...
				const shareMessage = `https://wa.me/?text=${window.encodeURIComponent(
					`Mira que bueno para ${property.p.operation_type == 'Venta' ? 'comprar' : 'alquilar'}! ${window.location.href}`
				)}`;
		...
		function getDevelopmentProps(id) {
			getFeaturedProperties.getDevelopmentProps(id).then(
				(result) => {
					...
					property.devProps.splice(
						property.devProps.findIndex((prop) => prop.id == property.p.id),
						1
					);
		...
		property.setActiveImage = (image) => {
			...
			property.showGalleryVideo = false;
			property.activeGalleryPhoto = image.image;
			property.galleryIndex = property.p.prop.photos.findIndex((i) => i == image) + 1;
		};

		property.setActiveVideo = (video) => {
			property.showDuit360 = false;
			property.showGalleryVideo = true;
			property.activeGalleryVideo = video;
		};
		...
		property.moveGallerySlider = (slider, side) => {
			...
			property.galleryIndex = sliderMoves.moveSliderByIndex(slider, property.galleryIndex, property.gallerySliderLength, side, width);
		};
