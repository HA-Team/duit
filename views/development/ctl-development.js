    ...
    getFeaturedProperties.getDevelopmentProps(id).then(result => {
      ...
      development.devProps = result.map(prop => {
        ...
        return {
          ...
          sell: prop.operations.filter(p => prop.operation_type == "Venta")[0] ? prop.operations.filter(p => prop.operation_type == "Venta")[0].prices.slice(-1)[0] : [],
          rent: prop.operations.filter(p => prop.operation_type == "Alquiler")[0] ? prop.operations.filter(p => prop.operation_type == "Alquiler")[0].prices.slice(-1)[0] : [],
